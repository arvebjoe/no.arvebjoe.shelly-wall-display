...
- Label and button text supports `$token` live values, resolved by the rendered GUI's runtime: built-ins `$time`/`$date` (extend `LABEL_VALUES` in `src/renderer.ts`) plus display variables pushed by the `set-variable` action card (WebSocket `variable` message). Variables are per device, kept in memory on the server (`deviceVariables`) and re-sent on WebSocket reconnect; an empty value clears one. Unset variables render as nothing on the display, but the raw `$token` stays visible in the editor preview. The legacy default GUI (`public/index.html`) has no labels and ignores `variable` messages.
